...
sudo docker run --rm \
  ...
  -v /home/ubuntu/models/Qwen3.6-35B-A3B-FP8/hub:/model_weights/hub:ro \
...
| Model loading | 16.91 s |
| └ Safetensors weight loading | 14.10 s |
| Post-load encoder/cache preparation | ~10.00 s |
| └ Cached Torch compilation loading | 0.24 s |
| └ Initial profiling/warmup | 7.35 s |
...
| API setup and application startup | ~20.00 s |
| └ Multi-modal warmup | 17.84 s |
| └ Read-only multi-modal warmup | 1.74 s |
| **Total: container log start → API ready** | **~112.00 s** |
